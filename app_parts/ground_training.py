...
def render():
    ...
    if 'correct_answer_index' not in st.session_state:
        st.session_state.correct_answer_index = None
    ...
    # 
    # Example structure:
    # question_text = gemini_api.get_question()
    ...
    question_text = "¿Cuál es el procedimiento correcto para el abordaje de pasajeros?"
    answer_options = [
        "Verificar documentación y dirigir a la puerta de embarque",
        "Permitir el acceso sin verificación",
        "Solicitar solo el boleto",
        "Revisar equipaje de mano únicamente"
    ]
    ...
    if st.session_state.correct_answer_index is None:
        st.session_state.correct_answer_index = correct_answer_index
    ...
    st.markdown(f"""
        <div class="question-box">
            <strong>Pregunta {st.session_state.current_question + 1}</strong><br><br>
            {question_text}
        </div>
    """, unsafe_allow_html=True)
    ...
    if st.session_state.answer_submitted:
        ...
        with col3:
            if st.button("Siguiente →", key="next_button", use_container_width=True):
    ...
    if st.session_state.answer_submitted:
        if st.session_state.selected_answer == st.session_state.correct_answer_index:
